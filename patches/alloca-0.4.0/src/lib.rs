...
//! (a plain, non-optional `[dev-dependencies]` entry of `benches/astrs-benches` and of several
//! crates' own micro-benchmarks -- `astrs-data`, `astrs-shm`, `astrs-rtps`, `astrs-wire`,
//! `astrs-transport`, `astrs-scheduler`): `criterion`'s measurement loop calls
//! `alloca::with_alloca` on every `Bencher::iter` invocation to vary the stack allocation size as
//! an anti-measurement-bias heuristic, so upstream's `alloca.c` was compiled on *every*
//! `cargo test`/`cargo bench`/`cargo deny check` pass over those crates' dev-dependencies --
//! independent of `--all-features`, and independent of ever actually running a benchmark.
//!
...
//! set through the address space just as effectively as a stack one does, and none of this
//! workspace's own benchmark assertions (§20.4) depend on `alloca`'s placement.
//!
//! Matches the identical fix already applied in the COOLJAPAN `scirs` workspace
//! (`patches/alloca-0.4.0`) for the same upstream defect; kept as a byte-for-byte copy less this
//! doc comment's crate references, so the two stay trivially diffable.
